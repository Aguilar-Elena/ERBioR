...
risk_rank <- function(x) {
  ...
  rank <- match(value, levels)

  if (length(rank) != 1L || is.na(rank)) {
    return(NA_integer_)
  }
...
find_response_unit <- function(response,
  ...
  # ----------------------------------------------------------
  # 1. Exact match: agent + canonical questionnaire ID
  # ----------------------------------------------------------
  ...
  exact_matches <- vapply(
    ...
    function(z) {
      ...
      q_ok <-
        if (!json_missing(gold_unit$questionnaire_id)) {
          same_chr(
            z$questionnaire_id,
  ...
  #
  # If questionnaire_id is non-canonical but there is exactly
  # one evaluation unit for the correct biological agent,
  # preserve the scientific unit. Questionnaire-ID mismatch is
  # handled as a traceability issue rather than treating the
  # entire assessment unit as absent.
  # ----------------------------------------------------------

  agent_matches <- vapply(
    candidate,
    function(z) {
  ...
  idx_agent <- which(agent_matches)

  if (length(idx_agent) == 1L) {
    return(candidate[[idx_agent]])
  }
  ...
  if (case_id == "ERBB-P04") {
    ...
    if (length(units) && length(gold_units)) {
      ...
      if (!is.null(ru) &&
        ...
        # U15 requires evidence that the reported compliance
        # percentage itself was calculated using the total number
        # of questionnaire items as denominator.
        #
        # A discordant n_applicable field alone is a structural/
        # traceability inconsistency and is NOT sufficient evidence
        # that not-applicable responses were actually included in
